...
Дизайн:
  ...
  * Single-token only: возвращаем только однословные кандидаты.
    RuWordNet валит в одну кучу синсеты «слово → словосочетание»
    (`телефон → номер телефона, телефонный аппарат`), гипонимы
    (`шина → автомобильная шина, грузовая шина`) и омонимы из чужих
    доменов (`шина → компьютерная шина, медицинская шина`).
    Маркетплейс-замена это «слово ↔ слово» — фразы тут только мусорят
    `expandable_tokens` в UI и мешают глазу. Однословный фильтр оставляет
    `шина → автопокрышка, авторезина, автошина` и режет всё остальное.
  * Контекстно-неверные однословные кандидаты (`шина → баллон`) добивает
    LM-фильтр на уровне expanded queries — там виден весь запрос.
        ...
        try:
            for ss in self._wn.get_synsets(word):
                for sense in ss.senses:
                    ...
                    if not name or name == word:
                        continue
                    # Single-token only. Дефис считаем частью слова
                    # (`x-ray`, `wi-fi`), пробел/подчёрк — нет.
